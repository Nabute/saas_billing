...
export class GenericService<T extends BaseEntity> {
    ...
    async destroy(id: string): Promise<void> {
        ...
        try {
            const entity = await queryRunner.manager.findOne(this.entity, { where: { id } as FindOptionsWhere<T> });
     ...
     */
    async saveEntityWithDefaultState(entity: T, defaultStateType: string): Promise<T> {
        if (!entity.id) {
