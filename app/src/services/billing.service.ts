...
export class BillingService {
    ...
    async scheduleInvoiceGeneration(): Promise<void> {
        ...
        const jobs = subscriptions.map(subscription =>
            this.billingQueue.add('generateInvoice', { subscriptionId: subscription.id })
        );
     ...
     */
    private async updateInvoiceAmount(subscriptionId: string, proratedAmount: number): Promise<void> {
    ...
    private async getInvoiceStatus(statusValue: string): Promise<DataLookup> {
        const status = await this.dataLookupRepository.findOne({ where: { value: statusValue } });
        if (!status) {
     ...
     * @throws NotFoundException if the subscription is not found.
     */
    private async findSubscriptionById(subscriptionId: string): Promise<CustomerSubscription> {
     ...
     */
    private async findSubscriptionPlanById(planId: string): Promise<SubscriptionPlan> {
