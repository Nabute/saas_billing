...
export class PaymentProcessor {
  ...
  async handleRetry(job: Job): Promise<void> {
    ...
    try {
      ...
      console.log(
        `Processing retry #${attempt} for subscription ID ${subscriptionId}`,
      );
      ...
      if (!result.success) {
        console.log(
          `Retry #${attempt} for subscription ID ${subscriptionId} failed`,
        );
        await this.paymentService.scheduleRetry(
          subscriptionId,
          attempt + 1,
          queryRunner.manager,
        );
      ...
      console.error(
        `Error during retry #${attempt} for subscription ID ${subscriptionId}:`,
        error.message,
      );
